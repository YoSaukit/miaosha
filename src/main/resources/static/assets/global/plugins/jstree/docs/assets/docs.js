...
(function ($) {
    ...
    $(function () {
        var search = function (str) {
                var tmp = str && str.length ? $.vakata.search(str, true, {
                        threshold: 0.2,
                        fuzzy: true,
                        caseSensitive: false
                    }) : null,
        ...
        //.state(window.location.protocol + '//' + window.location.host + window.location.pathname.replace(/^(.*?\/docs\/).*$/ig, '$1'))
            .init(function (e) {
                $('a:not([href^=http])').not($('.demo a')).address().on('click', function () {
        ...
        $.getJSON('./jstree.json', function (data) {
            ...
            $.each(data, function (ii, v) {
                ...
                if (name) {
                    ...
                    if (plugin) {
                        str += '<code class="meta plugin"><i class="glyphicon glyphicon-leaf"></i> ' + plugin + ' plugin</code> ';
                    }
                    if (priv) {
